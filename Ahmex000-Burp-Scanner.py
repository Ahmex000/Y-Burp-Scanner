...
class BurpExtender(IBurpExtender, IHttpListener):
    def registerExtenderCallbacks(self, callbacks):
        ...
        self._callbacks.setExtensionName("Ahmex000 Scanner")
...
KEYWORDS = {
    # General Secrets
   # General Secrets
r"(affirm|private|auth|system|api|root|linode|vultr|rackspace|digitalocean|firebase|gcs|aws|circleci|jenkins|travisci|teamcity|tumblr|reddit|pinterest|snap|acme|ssl|vpn)[-_]?(private|token|key|password|secret)": "High",
...
r"(sk|pk|stripe|paypal|square)[-_]?(live|test|api|key|token|client|secret)": "High",
"stripeAccountId": "Medium",

# Cloud Provider Secrets
r"(aws|gcp|azure|digitalocean|heroku)[-_]?(access|secret|session|service|account|storage|token|key)": "High",
r"secret[-_]?key|secretKey": "High",

# Database Credentials
r"(db|database|mongodb|postgresql|mysql)[-_]?(password|url|uri)": "High",

# Social Media Tokens
r"(facebook|twitter|instagram|linkedin)[-_]?(access|api|key|token)": "High",

# Version Control Tokens
r"(github|gitlab|bitbucket)[-_]?token": "High",

# CI/CD Secrets
r"(circleci|travisci|jenkins)[-_]?token": "High",

# Messaging Services
r"(slack|twilio|sendgrid)[-_]?(api|webhook|auth|token|key)": "High",

# Other Common Secrets
r"(encryption|private|public|ssh|pgp|rsa|ssl|jwt|session)[-_]?(key|certificate|secret)": "High",
